...
export default class Mytask extends React.Component {
  ...
  delete = (flag) => {
    global.listData = global.listData.filter((item) => item.title != flag);
    this.setState({
      status: !this.state.status,
    });
    message.success("标题为" + `${flag}` + "新闻删除成功哦");
  };
    ...
    return (
      <div>
        <List
          ...
          dataSource={global.listData}
          footer={
            <div>
              <b>新闻列表</b> footer part
            </div>
          }
          renderItem={(item) => (
            <List.Item
                ...
                <Comment
                  avatar={
                    <Avatar
                      ...
                      alt={localStorage.user[0].username}
                    />
                  }
                  content={
                    <Editor
